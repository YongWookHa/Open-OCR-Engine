...
def hard_negative_mining(pred, target, cfg):
    ...
    positive = np.where(cpu_target >= cfg.craft.THRESHOLD_POSITIVE)[0]
    negative = np.where(cpu_target <= cfg.craft.THRESHOLD_NEGATIVE)[0]
